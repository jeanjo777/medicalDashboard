...
import { Search, HelpCircle, Settings, List, Maximize2, AlertCircle, Copy, FileText, Plus, LogOut } from 'lucide-react'
...
export default function Dashboard() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#c5c3d1] flex items-center justify-center">
        <div className="rounded-2xl bg-[#f5f4f0] p-8 shadow-2xl">
      ...
      <div className="mx-auto max-w-[1400px] rounded-xl sm:rounded-2xl lg:rounded-3xl bg-[#f5f4f0] p-3 sm:p-4 lg:p-6 shadow-2xl">
        ...
        <header className="mb-3 sm:mb-4 lg:mb-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-3">
          <div className="flex items-center gap-1.5 sm:gap-2">
            <div className="inline-flex items-center justify-center h-7 w-7 sm:h-8 sm:w-8 rounded-lg bg-gradient-to-br from-emerald-500 to-emerald-600 shadow-sm">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M22 12h-4l-3 9L9 3l-3 9H2" />
              </svg>
            </div>
            <span className="text-sm sm:text-base font-semibold text-black">MediCare</span>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2 mr-1">
              <div className="text-right">
                <div className="text-[10px] sm:text-xs font-medium text-black">{doctor?.name}</div>
                <div className="text-[9px] sm:text-[10px] text-gray-500">{doctor?.specialty}</div>
              </div>
            </div>
            ...
            <button
              onClick={logout}
              title="Deconnexion"
              className="flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-full hover:bg-red-100 text-gray-600 hover:text-red-600 transition-colors"
            >
              <LogOut className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
            </button>
            <div className="h-7 w-7 sm:h-9 sm:w-9 overflow-hidden rounded-full bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center text-white text-xs sm:text-sm font-bold">
